...
export const PipelineStepper: React.FC<PipelineStepperProps> = ({
  ...
  return (
    <div className="w-full specular-card rounded-2xl p-4 sm:p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 text-xs">
        {stages.map((stage) => {
          ...
          let containerClass = 'bg-obsidian-900 border-white/5 text-slate-400';
          if (isFirst) {
            containerClass = 'bg-sky-950/30 text-sky-200 border-sky-500/40 shadow-glow-cyan font-medium';
          } else if (isMeasurement) {
            containerClass = 'bg-amber-950/25 text-amber-200 border-amber-500/40 font-medium';
          } else if (isBrief) {
            containerClass = isHumanApproved 
              ? 'bg-emerald-950/30 text-emerald-200 border-emerald-500/40 shadow-glow-emerald font-medium'
              : 'bg-emerald-950/15 text-emerald-300/90 border-emerald-500/30 font-medium';
          }

          return (
            <div
              key={stage.id}
              className={`p-2.5 rounded-xl border flex flex-col justify-between gap-2 text-[11px] transition-colors relative overflow-hidden group ${containerClass}`}
            >
